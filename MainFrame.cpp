...
void MainFrame::CreateControls()
{
	...
	headlineText->SetBackgroundColour(*wxYELLOW);//this sets color of to do you can change it too

	inputField = new wxTextCtrl(panel, wxID_ANY, "", wxPoint(100, 80), 
		wxSize(495, 35), wxTE_PROCESS_ENTER);//for enter button used wxTE_PROCESS_ENTER
	...
	//clearButton->SetBackgroundColour(*wxYELLOW);//this sets color of to do you can change it too
}

void MainFrame::BindEventHandlers()
{
	addButton->Bind(wxEVT_BUTTON, &MainFrame::OnAddButtonClicked, this);
	inputField->Bind(wxEVT_TEXT_ENTER, &MainFrame::OnInputEnter, this);//for enter button
	checkListBox->Bind(wxEVT_KEY_DOWN, &MainFrame::OnListKeyDown, this);//delete tasks
	clearButton->Bind(wxEVT_BUTTON, &MainFrame::OnClearButtonClicked, this);//what happens when clear is clicked
...
{
	AddTaskFromInput();//calling for enter button
}

void MainFrame::OnListKeyDown(wxKeyEvent& evt)//for deleting tasks
{
	switch (evt.GetKeyCode()) {
		case WXK_DELETE://if delete key is pressed only then proceed
			DeleteSelectedTask();
			break;
		case WXK_UP:
			MoveSelectedTask(-1);
...
void MainFrame::AddTaskFromInput()
{
	...
	if (!description.IsEmpty()) {
		checkListBox->Insert(description, checkListBox->GetCount());//enter the task
		inputField->Clear();//to clear the tab after the task is added
	}
	inputField->SetFocus();//to clear the tab after the task is added
}

void MainFrame::DeleteSelectedTask()
{
	int selectedIndex = checkListBox->GetSelection();//delete the item
